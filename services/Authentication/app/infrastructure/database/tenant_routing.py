"""Authentication's binding to the shared per-tenant connection router.

This is the thin, Authentication-specific glue: it constructs the shared
`TenantEngineRegistry` from Authentication's settings once, and hands out per-tenant
...
implementation (see TODO.md → "Build this once in shared/, not inside Authentication").
"""
...
def get_registry() -> TenantEngineRegistry:
    """Process-wide per-tenant engine registry, built from Authentication settings.
