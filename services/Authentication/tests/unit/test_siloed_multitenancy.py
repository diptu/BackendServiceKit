...
engine registry, tenant context, and Authentication's tenant-aware `get_db`.
...
async def test_resolver_substitutes_tenant_hex() -> None:
    resolver = TemplateConnectionResolver("postgresql+asyncpg://h/auth_{tenant}")
    tenant_id = uuid.uuid4()
    assert (
        await resolver.resolve(tenant_id)
        == f"postgresql+asyncpg://h/auth_{tenant_id.hex}"
    )
...
async def test_resolver_override_wins_over_template() -> None:
    ...
    resolver = TemplateConnectionResolver(
        "postgresql+asyncpg://h/auth_{tenant}",
        {str(tenant_id): "postgresql+asyncpg://other-host/auth_special"},
    )
    assert (
        await resolver.resolve(tenant_id)
        == "postgresql+asyncpg://other-host/auth_special"
    )
...
def _sqlite_registry(tmp_path: object, **kwargs: object) -> TenantEngineRegistry:
    template = f"sqlite+aiosqlite:///{tmp_path}/auth_{{tenant}}.db"
...
async def test_get_db_fails_closed_without_tenant(
    ...
    monkeypatch.setattr(
        ...
        "sqlite+aiosqlite:///./auth_{tenant}.db",
    )
...
async def test_get_db_routes_to_the_request_tenant_database(
    ...
    monkeypatch.setattr(
        settings,
        "tenant_database_url_template",
        f"sqlite+aiosqlite:///{tmp_path}/auth_{{tenant}}.db",
    )
