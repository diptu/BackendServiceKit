"""FastAPI application factory for the Storefront service.

A tenant-scoped e-commerce backend (catalog, cart, orders) built on the
platform's physical-silo multi-tenancy: each shop's data lives in that
tenant's own database, resolved per request from the X-Tenant-ID header
(see app/infrastructure/database/tenant_routing.py).
"""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # RabbitMQ — storefront domain events. Graceful degradation: a down broker
    # never blocks catalog/cart/order operations.
