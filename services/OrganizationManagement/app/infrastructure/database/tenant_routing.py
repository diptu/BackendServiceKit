"""OrganizationManagement's binding to the shared per-tenant connection router.

This is the thin, OrganizationManagement-specific glue: it constructs the shared
`TenantEngineRegistry` from OrganizationManagement's settings once, and hands out per-tenant
...
implementation (see TODO.md → "Build this once in shared/, not inside OrganizationManagement").
"""
...
def get_registry() -> TenantEngineRegistry:
    """Process-wide per-tenant engine registry, built from OrganizationManagement settings.
