...
pass tenant ids explicitly (argv) or as a comma-separated `ORG_TENANT_IDS`
env var.

Usage:
    uv run python -m scripts.tenant_migrations upgrade <tenant_id> [<tenant_id> ...]
    ORG_TENANT_IDS="<uuid>,<uuid>" uv run python -m scripts.tenant_migrations upgrade
...
def _tenant_ids(args: list[str]) -> list[UUID]:
    raw = args or [
        t for t in os.environ.get("ORG_TENANT_IDS", "").split(",") if t.strip()
    ]
    if not raw:
        raise SystemExit(
            "No tenant ids given (pass as args or set ORG_TENANT_IDS=uuid,uuid)."
        )
