...
response, not 404 — a user who exists in UserManagement but has never
touched their profile isn't an error. Writes 404 only if UserManagement
doesn't have this user_id at all (checked once, on first write — see
...
async def get_profile(user_id: UUID, svc: ProfileServiceDep) -> ProfileResponse:
...
async def delete_avatar(user_id: UUID, svc: AvatarServiceDep) -> None:
    await svc.delete_avatar(user_id)


@router.get("/avatar", response_model=AvatarResponse)
async def get_avatar(user_id: UUID, svc: AvatarServiceDep) -> AvatarResponse:
    avatar = await svc.get_avatar(user_id)
    if avatar is None:
...
async def get_preferences(user_id: UUID, svc: ProfileServiceDep) -> PreferencesResponse:
    preferences = await svc.get_preferences(user_id)
...
async def get_contacts(user_id: UUID, svc: ProfileServiceDep) -> ContactResponse:
    contact = await svc.get_contacts(user_id)
    if contact is None:
