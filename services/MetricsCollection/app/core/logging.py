...
package is importable. Falls back to a local formatter otherwise — `shared/`
lives at the repo root and is not copied into any service's Docker build
context today, so importing it can't be a hard dependency at runtime. Copied
from `services/Logging/app/core/logging.py` verbatim — see that file's
docstring for the full reasoning; the same constraint applies here unchanged.
"""
...
def configure_logging(*, debug: bool = False) -> None:
    try:
        ...
        _configure(debug=debug, service_name="metrics-collection")
        return
    except ImportError:
        pass
