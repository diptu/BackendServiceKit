...
from the active span, and is the formatter this service's stdout logs are
validated against in `tests/unit/test_promtail_contract.py`) when the
`shared/` package is importable. Falls back to a local formatter otherwise —
`shared/` lives at the repo root and is not copied into any service's Docker
build context today, so importing it can't be a hard dependency at runtime.
Tenent/APIGateway make the same allowance for `shared.observability.tracing`
(wrapped in try/except in their lifespans); this mirrors that for logging.
"""
...
def configure_logging(*, debug: bool = False) -> None:
    try:
        ...
        _configure(debug=debug, service_name="logging")
        return
    except ImportError:
        pass
