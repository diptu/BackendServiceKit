"""FastAPI application factory for the Logging service."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    app.state.http_client = httpx.AsyncClient(timeout=settings.loki_timeout)
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Logging — Tenant-scoped log query/ingest/export API",
...
def _register_exception_handlers(app: FastAPI) -> None:
    ...
    @app.exception_handler(InvalidLogIdError)
    @app.exception_handler(LogEntryNotFoundError)
    async def _not_found(request: Request, exc: Exception) -> JSONResponse:
        return JSONResponse(status_code=404, content={"detail": str(exc)})

    @app.exception_handler(InvalidLogQLError)
    @app.exception_handler(BulkIngestLimitExceededError)
    ...
    @app.exception_handler(LokiQueryError)
