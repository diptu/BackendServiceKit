"""FastAPI application factory for User.

Merges UserManagement + UserLifecycleManagement + UserProfileManagement
into one deployable service. RabbitMQ is still wired up here — IAM's
UserProjection is a real external consumer of the identity.events exchange
this service publishes to (see app/domain/events.py). What's gone is the
internal HTTP client dance UserLifecycleManagement/UserProfileManagement
used to need just to call *this* service — those are direct in-process
calls now (see app/services/profile_service.py, app/services/user_service.py).
"""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # waiting for since IAM's own build. Graceful degradation: a down
    # broker never blocks user CRUD, it just means events aren't published
    # (NullPublisher fallback, see app/api/v1/dependencies.py::get_publisher).
