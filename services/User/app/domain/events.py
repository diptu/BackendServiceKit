...
UserStatusChanged carries an `action` field distinguishing verbs that land
on the same to_status (activate vs onboard both end in ACTIVE, deactivate
vs offboard both end in DEACTIVATED) — merged from
UserLifecycleManagement's separate LifecycleTransitioned event, now that
there's only one status/one audit trail.
"""
