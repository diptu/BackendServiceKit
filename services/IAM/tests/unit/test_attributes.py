...
async def test_create_attribute_duplicate_key_same_user(client: AsyncClient) -> None:
    ...
    r1 = await client.post(
        f"/api/v1/user-attributes/{user_id}", json=body, headers=_headers(tenant_id)
    )
...
async def test_list_attributes(client: AsyncClient) -> None:
    ...
    r = await client.get(
        f"/api/v1/user-attributes/{user_id}", headers=_headers(tenant_id)
    )
...
async def test_update_attribute(client: AsyncClient) -> None:
    ...
    r2 = await client.patch(
        f"/api/v1/user-attributes/{user_id}/{attribute_id}",
        json={"value": 2},
        headers=_headers(tenant_id),
    )
