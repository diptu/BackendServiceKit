...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_tenants_status_region", "tenants", ["status", "region"], unique=False
    )
    op.create_table(
        "tenant_contacts",
        sa.Column("id", sa.Uuid(), nullable=False),
        ...
        sa.Column(
            "role", sa.String(length=50), nullable=False, comment="owner | admin"
        ),
        sa.Column(
            "added_at",
            sa.DateTime(timezone=True),
        ...
        sa.ForeignKeyConstraint(
            ["tenant_id"],
            ["tenants.id"],
            name=op.f("fk_tenant_contacts_tenant_id_tenants"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_tenant_contacts")),
    )
    op.create_index(
        "idx_tenant_contacts_active",
        "tenant_contacts",
        ["tenant_id", "removed_at"],
        unique=False,
    )
    op.create_index(
        "idx_tenant_contacts_tenant_user",
        "tenant_contacts",
        ["tenant_id", "user_id"],
        unique=True,
    )
    op.create_index(
        op.f("ix_tenant_contacts_tenant_id"),
        "tenant_contacts",
        ["tenant_id"],
        unique=False,
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["tenant_id"],
            ["tenants.id"],
            name=op.f("fk_tenant_metadata_tenant_id_tenants"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_tenant_metadata")),
    )
    op.create_index(
        "idx_tenant_metadata_tenant_key",
        "tenant_metadata",
        ["tenant_id", "key"],
        unique=True,
    )
    op.create_index(
        op.f("ix_tenant_metadata_tenant_id"),
        "tenant_metadata",
        ["tenant_id"],
        unique=False,
    )
